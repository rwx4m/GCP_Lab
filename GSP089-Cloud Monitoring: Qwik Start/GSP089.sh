...
gcloud compute scp apache_install.sh lamp-1-vm:/tmp --project=$DEVSHELL_PROJECT_ID --zone=$ZONE --quiet

gcloud compute ssh lamp-1-vm --project=$DEVSHELL_PROJECT_ID --zone=$ZONE --quiet --command="bash /tmp/apache_install.sh"
...
gcloud beta monitoring channels create --channel-content-from-file="notif_email.json"
...
gcloud alpha monitoring policies create --policy-from-file="log.json"
